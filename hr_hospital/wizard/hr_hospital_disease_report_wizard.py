...
class DiseaseReportWizard(models.TransientModel):
    ...
    def action_generate_report(self):
        ...
        if not self.date_from or not self.date_to:
            raise ValidationError(_("Будь ласка, вкажіть обидві дати: 'З' та 'По'"))
        ...
        report_data = {
            'form_data': {
                ...
                'doctor_names': ', '.join(self.doctor_ids.mapped(
                    'name')) if self.doctor_ids else 'Усі лікарі',
                'disease_names': ', '.join(self.disease_ids.mapped(
                    'complete_name')) if self.disease_ids else 'Усі хвороби',
            },
            'diagnoses': []
        }

        # Формування домену для пошуку діагнозів
        domain = [
        ...
        for diagnosis in self.env['hr.hospital.diagnosis'].search(domain):
            report_data['diagnoses'].append({
                ...
                'visit_date': diagnosis.visit_id.visit_date.strftime(
                    '%Y-%m-%d'),
                'details': diagnosis.description or '-'
            })

        if not report_data['diagnoses']:
            raise ValidationError(_("Не знайдено діагнозів за вказаними критеріями"))
